...
export const BasicInfo = ({ form }: BasicInfoProps) => {
  const [activeInput, setActiveInput] = useState<InputKey>("livingroom");

  const [previews, setPreviews] = useState<Record<InputKey, string[]>>(() => {
    return INPUT_KEYS.reduce((acc, key) => {
      acc[key] = [];
      return acc;
    }, {} as Record<InputKey, string[]>);
  });

  const fileInputRefs = useRef<Record<InputKey, RefObject<HTMLInputElement>>>(
    {} as any
  );
  ...
  useEffect(() => {
    INPUT_KEYS.forEach((key) => {
      if (!fileInputRefs.current[key]) {
        fileInputRefs.current[key] = React.createRef<HTMLInputElement>();
      }
    });
  }, []);
  ...
  const removeImage = (fieldName: InputKey, index: number) => {
    const files: File[] = form.getValues(fieldName) || [];
  ...
  const handleChange = (
    ...
    const existingFiles: File[] = form.getValues(fieldName) || [];
  ...
  const preview = useMemo(() => {
    const files: File[] = form.getValues(activeInput);
  ...
  useEffect(() => {
    ...
    const filteredInputs = INPUT_KEYS.filter(
